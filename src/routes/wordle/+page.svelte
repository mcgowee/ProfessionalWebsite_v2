<script lang="ts">
  ...
  function normGuess(s: string) {
    return s.toLowerCase().replace(/[^a-z]/g, '').slice(0, 5);
  }
  ...
  function setGuessChar(rowIndex: number, colIndex: number, value: string) {
    const ch = (value || '').toLowerCase().replace(/[^a-z]/g, '').slice(0, 1);
...
<main class="wrap">
  <header class="top">
    ...
    <p class="intro">
      ...
      <strong>b</strong>=blank, <strong>y</strong>=yellow, <strong>g</strong>=green.
      It then ranks remaining words by <strong>letter frequency</strong> (favoring words with many unique, common letters).
    </p>
    ...
    <div class="how">
      ...
      <ul>
        ...
        <li>Click <strong>Solve</strong> to get suggestions. A new blank row is added automatically.</li>
...
<style>
  ...
  .top h1 { margin: 0 0 8px; font-size: 34px; letter-spacing: -0.02em; }
  ...
  .tile:focus { box-shadow: 0 0 0 3px rgba(17,24,39,0.12); border-color: rgba(0,0,0,0.55); }

  .tile.b { background: #e5e7eb; color: #111; }
  .tile.y { background: #f59e0b; color: #111; }
  .tile.g { background: #16a34a; color: #fff; }

  .msg { margin-top: 12px; padding: 10px 12px; border-radius: 10px; font-weight: 600; }
  .msg.error { background: #fee2e2; border: 1px solid #fecaca; color: #7f1d1d; }

  .results { margin-top: 16px; border-top: 1px solid rgba(0,0,0,0.12); padding-top: 14px; }
  .summary { margin-bottom: 10px; color: #111; }

  .results h2 { margin: 10px 0 10px; font-size: 18px; }
  ...
  .suggestion:focus { outline: none; box-shadow: 0 0 0 3px rgba(17,24,39,0.12); }

  .word { font-weight: 900; letter-spacing: 0.08em; text-transform: uppercase; }
  .score { color: var(--muted, #666); font-weight: 700; white-space: nowrap; }
</style>
