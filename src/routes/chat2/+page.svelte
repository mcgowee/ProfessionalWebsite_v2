		...
		messages = [
			{
				sender: 'AI',
				type: 'info',
				text: 'Ask a question about the Chinook database. This demo generates SQL under read-only constraints and returns structured results.'
			}
		];
	});
		...
		messages = [
			{
				sender: 'AI',
				type: 'info',
				text: 'Chat cleared. Ask a question about the Chinook database. This demo generates SQL under read-only constraints and returns structured results.'
			}
		];
	}
	...
	async function sendMessage() {
		...
		try {
			...
			if (response.ok) {
				...
				messages = [
					...messages,
					{ sender: 'AI', type: 'error', text: `Error: ${data?.error || 'Request failed.'}` }
				];
			}
		} catch {
			messages = [
...
<main class="page">
	<div class="hero-wrapper">
		<img
			src="/assets/sql_chat_hero.jpg"
			alt="SQL Chat interface visualization"
			class="hero-img-banner"
		/>
	</div>
	<header class="hero">
		<div class="hero__left">
			...
			<p class="subtitle">
				Natural language → <strong>read-only</strong> SQL queries on a relational database, with structured
	...
	<section class="db">
		<div class="db__card">
			<div class="db__text">
				...
				<p class="p">
					Chinook is a sample relational database representing a digital media store (artists,
					albums, tracks, invoices, and customers). It’s ideal for demonstrating SQL analytics
					patterns.
				</p>
				<p class="p">
					Reference:
					<a
						class="link"
						href="https://www.sqlitetutorial.net/sqlite-sample-database/"
						target="_blank"
		...
		<div class="samples">
			...
			<div class="samples__grid">
				<button
					type="button"
					class="chip"
					on:click={() => setSampleQuestion('What are the genres?')}
				>
					What are the genres?
				</button>
				<button
					...
					class="chip"
					on:click={() =>
						setSampleQuestion('List the top ten artists with the most albums and add a count.')}
				>
				...
				<button
					type="button"
					class="chip"
					on:click={() => setSampleQuestion('Show me all albums by Led Zeppelin.')}
				>
				...
				<button
					type="button"
					class="chip"
					on:click={() => setSampleQuestion('What is the average invoice total?')}
				>
	...
	<section class="chat">
		<div class="chat__header">
			...
			<div class="chat__actions">
				...
				<button class="btn btn--ghost" type="button" on:click={clearChat} disabled={isLoading}
					>Clear</button
				>
			</div>
		</div>
		...
		<form class="inputRow" on:submit|preventDefault={sendMessage}>
			...
			<textarea
				...
				disabled={isLoading}
			></textarea>
			<button class="btn btn--primary" type="button" on:click={sendMessage} disabled={isLoading}
				>Send</button
			>
		</form>

		<p class="foot">
			This demo is designed for analytics exploration. It uses read-only constraints and limits
			result sizes to reduce risk and improve performance.
		</p>
	</section>
</main>
...
<style>
	.page {
		...
		background:
			...
			radial-gradient(700px 500px at 85% 10%, rgba(255, 123, 189, 0.14), transparent 55%), var(--bg);
		min-height: 100vh;
	...
	.msg--ai {
		background: rgba(255, 123, 189, 0.1);
		border-color: rgba(255, 123, 189, 0.24);
	}

	.msg--error {
		background: rgba(199, 37, 37, 0.1);
